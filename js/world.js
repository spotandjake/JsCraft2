...
// createWorld()
World.prototype.createWorld = function( height )
{
	this.spawnPoint = new Vector( this.sx / 2 + 0.5, this.sy / 2 + 0.5, height );
...
{
	if (type == BLOCK.AIR && this.blocks[x][y][z].id == BLOCK.TNT.id && this.blocks[x][y][z].explode == false) {
		var newstate= JSON.parse(JSON.stringify(this.blocks[x][y][z]));
		newstate.explode = true;
		newstate.texture = BLOCK.fromId(newstate.id).texture;
		this.blocks[x][y][z] = newstate;
		if ( this.renderer != null ) this.renderer.onBlockChanged( x, y, z );
	} else if (this.blocks[x][y][z] != BLOCK.BEDROCK) {
...
World.prototype.saveToClip = function() {
	var data = this.spawnPoint.x + "," + this.spawnPoint.y + "," + this.spawnPoint.z + "," + this.toNetworkString();
	return data;
}
World.prototype.loadFromStr = function( content, compression ) {
	try {
...
if ( typeof( exports ) != "undefined" )
{
	...
	World.prototype.loadFromFile = function( filename, compression ) {
		var fs = require( "fs" );
		try {
			fs.lstatSync( filename );
			var data = fs.readFileSync( filename, "utf8" );
			var hm = new compression.Hauffman();
			var data = hm.decompress(data).split( "," );
			this.createFromString( data[3] );
			this.spawnPoint = new Vector( parseInt( data[0] ), parseInt( data[1] ), parseInt( data[2] ) );
			return true;
		} catch ( e ) {
	...
	World.prototype.saveToFile = function( filename, compression  ) {
		var data = this.spawnPoint.x + "," + this.spawnPoint.y + "," + this.spawnPoint.z + "," + this.toNetworkString();
		var hm = new compression.Hauffman();
		var compressed = hm.compress(data);
		// console.log(`Compressed: ${compressed.length}`)
		// console.log(`Uncompressed: ${data.length}`)
		// console.log(`Compression Rate: ${data.length/compressed.length}`)
		require( "fs" ).writeFileSync( filename, compressed);
		require( "fs" ).writeFileSync( "world_uncompressed.jscraft", data);
	}
