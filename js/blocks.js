...
BLOCK.TORCH = {
	...
	texture: function(world, lightmap, lit, x, y, z, dir) {
		if (dir == DIRECTION.UP)
			return [7 / 256, 86 / 256, 9 / 256, 88 / 256];
		else if (dir == DIRECTION.DOWN)
			return [7 / 256, 88 / 256, 9 / 256, 90 / 256];
		else
			return [7 / 256, 86 / 256, 9 / 256, 96 / 256];
	}
};
...
BLOCK.pushVertices = function(vertices, world, lightmap, x, y, z) {
	...
	var bH = block.fluid && (z == world.sz - 1 || !blocks[x][y][z + 1].fluid) ? 0.9 * block.stage : 1.0;
	if (block.torch == true) {
		// Top
		if (z == world.sz - 1 || world.blocks[x][y][z + 1].transparent || block.fluid) {
			...
			pushQuad(
				vertices,
				[x + 0.4, y + 0.4, z + 0.9, c[0], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.4, z + 0.9, c[2], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.6, z + 0.9, c[2], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.6, z + 0.9, c[0], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0]
			);
		}
		// Bottom
		if (z == 0 || world.blocks[x][y][z - 1].transparent) {
			...
			pushQuad(
				vertices,
				[x + 0.6, y + 0.6, z, c[0], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.4, z, c[2], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.4, z, c[2], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.6, z, c[0], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0]
			);
		}
		// Front
		if (y == 0 || world.blocks[x][y - 1][z].transparent) {
			...
			pushQuad(
				vertices,
				[x + 0.4, y + 0.4, z, c[0], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.4, z, c[2], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.4, z + 0.9, c[2], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.4, z + 0.9, c[0], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0]
			);
		}
		...
		if (y == world.sy - 1 || world.blocks[x][y + 1][z].transparent) {
			...
			pushQuad(
				vertices,
				[x + 0.4, y + 0.6, z + 0.9, c[2], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.6, z + 0.9, c[0], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.6, z, c[0], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.6, z, c[2], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0]
			);
		}
		// Left
		if (x == 0 || world.blocks[x - 1][y][z].transparent) {
			...
			pushQuad(
				vertices,
				[x + 0.4, y + 0.4, z + 0.9, c[2], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.6, z + 0.9, c[0], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.6, z, c[0], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.4, y + 0.4, z, c[2], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0]
			);
		}
		...
		if (x == world.sx - 1 || world.blocks[x + 1][y][z].transparent) {
			...
			pushQuad(
				vertices,
				[x + 0.6, y + 0.4, z, c[0], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.6, z, c[2], c[3], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.6, z + 0.9, c[2], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0],
				[x + 0.6, y + 0.4, z + 0.9, c[0], c[1], lightMultiplier, lightMultiplier, lightMultiplier, 1.0]
			);
		}
	} else {
		// Top
